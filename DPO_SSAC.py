...
from convlab.policy.rlmodule_sac import MultiDiscretePolicy, Value, MemoryReplay
...
class Discrete_SAC(Policy):

    def __init__(self, is_train=False, dataset='Multiwoz', seed=0, vectorizer=None, load_path=""):
        
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'configs','sac_config.json'), 'r') as f:
            cfg = json.load(f)
        ...
        logging.info('sac seed ' + str(seed))
        set_seed(seed)
    ...
    def update(self, env, policy_sys, epoch):
        ...
        for i in range(self.training_iter):
            ...
            logging.debug('<<dialog critic1 SAC>> epoch {}, iteration {}, critic1, loss {}'.format(epoch, i, critic1_loss))
            logging.debug('<<dialog critic2 SAC>> epoch {}, iteration {}, critic2, loss {}'.format(epoch, i, critic2_loss))
            logging.debug('<<dialog actor SAC>> epoch {}, iteration {}, actor, loss {}'.format(epoch, i, actor_loss))
            logging.debug('<<dialog alpha SAC>> epoch {}, iteration {}, alpha, loss {}'.format(epoch, i, alpha_loss))
        ...
        logging.debug('<<dialog total critic1 SAC>> epoch {}, total_critic1_loss {}'.format(epoch, total_critic1_loss))
        total_critic2_loss  /= ( self.training_iter)
        logging.debug('<<dialog total critic2 SAC>> epoch {}, total_critic2_loss {}'.format(epoch, total_critic2_loss))
        total_actor_loss /= ( self.training_iter)
        logging.debug('<<dialog total actor SAC>> epoch {}, total_actor_loss {}'.format(epoch, total_actor_loss))
        total_alpha_loss  /= ( self.training_iter)
        logging.debug('<<dialog total alpha SAC>> epoch {}, total_alpha_loss {}'.format(epoch, total_alpha_loss))
        #print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    ...
    def save(self, directory, epoch):
        ...
        torch.save(self.critic_local.state_dict(), directory + '/' + str(epoch) + '_SAC.critic1.mdl')
        torch.save(self.critic_local_2.state_dict(), directory + '/' + str(epoch) + '_SAC.critic2.mdl')
        torch.save(self.actor.state_dict(), directory + '/' + str(epoch) + '_SAC.pol.mdl')
    ...
    def load(self, filename):
        critic1_mdl_candidates = [
            ...
            filename + '_SAC.critic1.mdl',
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '.critic1.mdl'),
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '_SAC.critic1.mdl'),
        ]
        ...
        critic2_mdl_candidates = [
            ...
            filename + '_SAC.critic2.mdl',
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '.critic2.mdl'),
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '_SAC.critic2.mdl')
        ]
        ...
        actor_mdl_candidates = [
            filename + '.pol.mdl',
            filename + '_SAC.pol.mdl',
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '.pol.mdl'),
            os.path.join(os.path.dirname(os.path.abspath(__file__)), filename + '_SAC.pol.mdl')
        ]
    ...
    def load_from_pretrained(self, archive_file, model_file, filename):
        ...
        if not os.path.exists(model_dir):
            os.mkdir(model_dir)
        if not os.path.exists(os.path.join(model_dir, 'best_sac.pol.mdl')):
